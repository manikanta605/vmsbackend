...
exports.tbl_voilations = async (data,insertViolationTitle, violationTitleInsertId,referenceNumber,callback) => {
    ...
    // console.log("referenceNumber",referenceNumber);
       var sql = "INSERT INTO `tbl_voilations`(`reference_number`,`side_type`, `violation_title_id`, `side_code`, `document_type`, `document_no`,`description_side_code`,`license_no`,`license_plate_no`,`plate_source`,`plate_code`,`plate_color`,`old_code`,`fine_category`,`fine_code`,`fine_place`,`area`,`fine_no`,`notes`,`recipient_person`,`phone`,`recipient_mobile`,`email`,`reserved_code`,`identity_doc`,`address`,`description`,`daily_fines`,`violation_docs`,`fine_amount`,`front_image`,`plate_category`,`source`,`source_id`,`created_by`,`created_on`) VALUES ('" + referenceNumber + "','" + data.voilationType + "','" + insertViolationTitle + "','" +violationTitleInsertId + "','" + data.documentType + "','" + data.documentNo + "','" + data.sideCodeDescription + "','" + data.licenseNo + "','" + data.plateNo + "','" + data.plateSource + "','" + data.plateCode + "','" + data.plateColor + "','" + data.oldCode + "','" + data.violationCategory + "','" + data.fineCode + "','" + data.finePlace + "','" + data.area + "','" + data.fineNo + "','" + data.fineNotes + "','" + data.recipientperson + "','" + data.phone + "','" + data.recipientmobile + "','" + data.email + "','" + data.reservedCode + "','" + data.reservedIdNumber + "','" + data.address + "','" + data.description + "','" + data.dailyFines + "','" + data.violationDocument + "','" + data.fineAmount + "','" + data.camerafiles + "','1','MANUAL','2','" + data.created_by + "','" + data.created_on + "')";
